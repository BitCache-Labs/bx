...
#if defined(BX_APP_GFX_OPENGL) || defined(BX_APP_GFX_OPENGLES)
#include <glad/glad.h>
...
static void glfw_error_callback(const i32 error, cstring desc)
{
	...
	bx_error(bx, "GLFW Error {}: {}", error, desc);
}
...
bool bx::dvc_init(const app_config_t& config) bx_noexcept
{
	...
	if (!glfwInit())
	{
		bx_error(bx, "Failed to init GLFW");
		return false;
	}

#ifdef BX_APP_GFX_VULKAN
	glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
#endif // BX_APP_GFX_VULKAN
	...
	if (!g_window)
	{
		bx_error(bx, "Failed to create GLFW window");
		glfwTerminate();
		return false;
	}
	...
	if (!gl_init(config))
	{
		bx_error(bx, "Failed to initialize GLFW graphics.");
		return false;
	}
#endif
	...
	if (!vk_init(config))
	{
		bx_error("Failed to initialize GLFW graphics.");
...
static bool gl_init(const bx::app_config_t& config)
{
...
#endif
	{
		bx_error(bx, "Failed to load GLAD");
		return false;
	}
	...
	{
		bx_error(bx, "Failed to initialize ImGui GLFW backend!");
		return false;
	}
...
static void vk_check_result(const VkResult err)
{
	...
	if (err != VK_SUCCESS)
	{
		bx_error("[Vulkan] Error: VkResult = {}", err);
		if (err < 0)
			abort();
	}
}
...
static bool vk_init(const bx::app_config_t& config)
{
	...
	{
		bx_error("Failed to get Vulkan support");
		glfwTerminate();
		return result_t::FAIL;
	}
