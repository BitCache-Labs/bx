#include <engine/common/common_file.hpp>
...
BX_MODULE_DEFINE(CommonFile)
BX_MODULE_DEFINE_INTERFACE(File, CommonFile)

bool CommonFile::Initialize()
{
...
void CommonFile::AddMountPoint(StringView mountName, StringView path)
{
    if (!Exists(path))
    {
...
bool CommonFile::Exists(StringView path)
{
...
u64 CommonFile::LastWrite(StringView filename)
{
#if (defined(BX_PLATFORM_PC) || defined(BX_PLATFORM_LINUX))
...
StringView CommonFile::GetFilename(StringView file)
{
...
FilePath CommonFile::GetPath(StringView filename)
{
    FileName category{};
    FileName postfix{};
    ...
    // Only category provided, search for existing files
    for (const auto& entry : mountList)
    {
        FilePath tmp = ReplaceMountPoint(filename, entry.path);
        if (Exists(tmp))
            return ReplaceMountPoint(filename, entry.path);
    }

    // No file exists, use default path
...
StringView CommonFile::GetExt(StringView filename)
{
    return filename.substr(filename.find_last_of(".") + 1);
}

StringView CommonFile::RemoveExt(StringView filename)
{
    return filename.substr(0, filename.find_last_of("."));
}

List<StringView> CommonFile::SplitPath(StringView path, char delimiter)
{
...
bool CommonFile::Move(StringView oldPath, StringView newPath)
{
...
bool CommonFile::Delete(StringView filename)
{
#if defined(BX_PLATFORM_PC)
...
bool CommonFile::CreateDirectory(StringView path)
{
#if defined(BX_PLATFORM_PC)
...
bool CommonFile::ListFiles(StringView root, List<FileHandle>& files)
{
#if defined(BX_PLATFORM_PC)
...
bool CommonFile::Find(StringView root, StringView filename, FilePath& filepath)
{
    ...
    for (const auto& file : files)
    {
        if (file.isDirectory)
        {
            if (CommonFile::Find(file.filepath, filename, filepath))
            {
                return true;
            }
        }
...
void CommonFile::FindEach(StringView root, StringView ext, const FindEachCallback& callback)
{
    List<FileHandle> files;
    if (!ListFiles(root, files))
        return;
...
String CommonFile::ReadText(StringView filename)
{
    const FilePath filepath = GetPath(filename);

    // Open the file in input mode.
    InputFileStream ifs(filepath, std::ios::in);
    if (!ifs.is_open())
    {
        BX_LOGE(File, "Failed to open file: {}", filepath);
        return String{};
    }
    ...
    if (!ifs)
    {
        BX_LOGE(File, "Failed to read from file: {}", filepath);
        return String{};
    }
...
bool CommonFile::WriteText(StringView filename, StringView text)
{
    const FilePath filepath = GetPath(filename);

    // Open the file in write mode (creating it if it doesn't exist)
    OutputFileStream ofs(filepath, std::ios::out | std::ios::trunc);
    if (!ofs.is_open())
    {
        BX_LOGE(File, "Failed to open file: {}", filepath);
        return false;
    ...
    if (!ofs)
    {
        BX_LOGE(File, "Failed to write to file: {}", filepath);
        return false;
    }
...
List<char> CommonFile::ReadBinary(StringView filename)
{
    const FilePath filepath = GetPath(filename);

    InputFileStream file(filepath, std::ios::binary | std::ios::ate);
    if (!file.is_open())
    {
        BX_LOGE(File, "File {} with full path {} was not found!", filename, filepath);
        return List<char>();
    }
