...
namespace bx
{
	...
	u32 g_queue_family                 = static_cast<u32>(-1);
	...
	VkCommandBuffer g_imgui_cmd_buffer = VK_NULL_HANDLE;
	VkPipeline g_imgui_pipeline        = VK_NULL_HANDLE;
	VkRenderPass g_imgui_render_pass   = VK_NULL_HANDLE;
...
bx::result_t bx::app_init() noexcept
{
...
#ifdef BX_VULKAN_SUPPORTED
	g_use_vulkan = strcmp(gfx_backend_name(), "vulkan") == 0;
#else
	g_use_vulkan = false;
#endif
	...
	const i32 win_width   = app_config_has<i32>("win_width") ? *app_config_get<i32>("win_width") : 800;
	const i32 win_height  = app_config_has<i32>("win_height") ? *app_config_get<i32>("win_height") : 600;
	const char* win_title = app_config_has<char>("win_title") ? app_config_get<char>("win_title") : "Title";
	const bool vsync      = true; // TODO: Get from config

	const f32 main_scale = ImGui_ImplGlfw_GetContentScaleForMonitor(glfwGetPrimaryMonitor()); // Valid on GLFW 3.3+ only
	const i32 scaled_width = static_cast<i32>(main_scale * static_cast<f32>(win_width));
	const i32 scaled_height = static_cast<i32>(main_scale * static_cast<f32>(win_height));
	g_window = glfwCreateWindow(scaled_width, scaled_height, win_title, nullptr, nullptr);
	if (!g_window)
	{
	...
	else
	{
		...
		{
			std::cerr << "Failed to get Vulkan support\n";
			glfwTerminate();
			return result_t::FAIL;
		}
...
void bx::app_end_frame(const bool present, const bool should_close) noexcept
{
	...
	glfwSetWindowShouldClose(g_window, glfwWindowShouldClose(g_window) || should_close);
}
...
bx::key_state bx::app_key(const i32 key) noexcept
{
	key_state state{};
	if (key >= 0 && key <= GLFW_KEY_LAST)
	{
...
bx::mouse_state bx::app_mouse() noexcept
{
	mouse_state s{};
